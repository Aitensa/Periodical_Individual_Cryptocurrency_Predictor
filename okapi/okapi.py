...
def get_data(uri, instrument_id, num): 
    ...
    unixtime = datetime.utcnow().isoformat()
    print(unixtime)
    sign_str = '%s%s%s%s' % (unixtime, method, uri, secretkey)
    ...
    # 使用sock5代理
    proxy = SOCKSProxyManager('socks5h://127.0.0.1:7890')
...
def get_recent(csvpath, instrument_id='ETH-USDT', gap=3600, num=128): 
    ...
    param = 'instId=%s'%instrument_id

    url = '%s?%s'%(uri, param)
    
    X = get_data(url, instrument_id, num)
    with open(csvpath, 'w') as f:
        f.write("time,open,high,low,close,volume,volCcy\n")
        for x in X[::-1]: # 逆序
            s = ','.join(x)
            f.write('%s\n'%s)
